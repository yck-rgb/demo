...
function loadBaseLayerPickerOfToolbar() {
  //图层选择需要汉化对象
  let layers = getChineseMaps();

  //获取容器
  let dropDownContainer = document.getElementsByClassName(
  ...
  if (dropDownContainer.length > 0) {
    ...
    let selectLayerToolbar = document.getElementsByClassName(
      "cesium-baseLayerPicker-selected"
    );
    if (selectLayerToolbar.length > 0) {
      selectLayerToolbar[0].parentElement.onmouseover = function (event) {
        let currentTitle = event.currentTarget.getAttribute("title");
        let currentTargets = currentTitle.split("\n");
        let tempTitle1 = "";
        let tempTitle2 = "";
...
function loadToolBarHelp() {
  let texts = getChineseMaps();

  //鼠标
  let mouseButton = document.getElementsByClassName(
  ...
  let touchButton = document.getElementsByClassName(
...
function loadFlyController() {
  ...
  if (parentContainer.length > 0) {
    ...
    let datedes = parentContainer.getElementsByClassName(
      "cesium-animation-svgText"
    );
    for (let i = 0; i < datedes.length; i++) {
      let text = datedes[i].children[0].innerHTML;
  ...
  //修改底部描述
  let cesiumDescription = document.getElementsByClassName(
    "cesium-credit-textContainer"
  );
  if (cesiumDescription.length > 0) {
    let insertEvent = function () {
      // let insertEvent = function (e) {
      // e = e || event;
  ...
  let timeLineBar = document.getElementsByClassName("cesium-timeline-bar");
  if (timeLineBar.length > 0) {
    ...
    timeLineBar.addEventListener("DOMNodeInserted", function (e) {
      ...
      if (e.target.getAttribute("class") == "cesium-timeline-ticLabel") {
        let texts = e.target.innerHTML.split(" ");
        if (texts.length > 3) {
          let resultText =
            texts[2] +
            "年" +
    ...
    let labels = timeLineBar.getElementsByClassName("cesium-timeline-ticLabel");
    for (let o = 0; o < labels.length; o++) {
      let longTexts = labels[o].innerHTML.split(" ");
...
function updateHtml(parent, map, className) {
  ...
  let doms = parent.getElementsByClassName(className);
...
function updateHtmlByTagName(parent, map, tagName) {
  ...
  let doms = parent.getElementsByTagName(tagName);
  for (let i = 0; i < doms.length; i++) {
    if (map[doms[i].innerHTML]) doms[i].innerHTML = map[doms[i].innerHTML];
  }
}
  ...
  if (doms.length > 0) {
    for (let i = 0; i < doms.length; i++) {
      doms[i][eventName] = function (event) {
        let attr = event.currentTarget.getAttribute(attributeName);
        if (map[attr])
