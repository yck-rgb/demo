...
function blur(values, r) {
  ...
  blur6(values, temp, 0, length3, 1);
  blur6(temp, values, 0, length3, 1);
  blur6(values, temp, 0, length3, 1);
  return values;
}
...
function blurh(blur6, T, S, w, h) {
  for (let y4 = 0, n = w * h; y4 < n; ) {
    blur6(T, S, y4, y4 += w, 1);
  }
}
function blurv(blur6, T, S, w, h) {
...
function blurfImage(radius) {
  ...
  return (T, S, start2, stop, step) => {
    ...
    blur6(T, S, start2 + 0, stop + 0, step);
    blur6(T, S, start2 + 1, stop + 1, step);
    blur6(T, S, start2 + 2, stop + 2, step);
    blur6(T, S, start2 + 3, stop + 3, step);
  };
}
function blurf(radius) {
  ...
  return (T, S, start2, stop, step) => {
    if (!((stop -= step) >= start2))
      return;
    let sum7 = radius0 * S[start2];
    const s0 = step * radius0;
    const s1 = s0 + step;
    for (let i = start2, j = start2 + s0; i < j; i += step) {
      sum7 += S[Math.min(stop, i)];
    }
    for (let i = start2, j = stop; i <= j; i += step) {
      sum7 += S[Math.min(stop, i + s0)];
      T[i] = (sum7 + t * (S[Math.max(start2, i - s1)] + S[Math.min(stop, i + s1)])) / w;
      sum7 -= S[Math.max(start2, i - s0)];
    }
  };
}
function bluri(radius) {
  ...
  return (T, S, start2, stop, step) => {
    if (!((stop -= step) >= start2))
      return;
    let sum7 = radius * S[start2];
    const s2 = step * radius;
    for (let i = start2, j = start2 + s2; i < j; i += step) {
      sum7 += S[Math.min(stop, i)];
    }
    for (let i = start2, j = stop; i <= j; i += step) {
      sum7 += S[Math.min(stop, i + s2)];
      T[i] = sum7 / w;
      sum7 -= S[Math.max(start2, i - s2)];
    }
  };
}
...
function reducer(reduce5) {
  return (values) => reduce5(...values);
}
function cross(...values) {
  const reduce5 = typeof values[values.length - 1] === "function" && reducer(values.pop());
  values = values.map(arrayify);
  const lengths = values.map(length);
  const j = values.length - 1;
  const index6 = new Array(j + 1).fill(0);
  const product = [];
  if (j < 0 || lengths.some(empty))
    return product;
  while (true) {
    product.push(index6.map((j2, i2) => values[i2][j2]));
    let i = j;
    while (++index6[i] === lengths[i]) {
      if (i === 0)
        return reduce5 ? product.map(reduce5) : product;
      index6[i--] = 0;
    }
  }
}
...
function cumsum(values, valueof) {
  var sum7 = 0, index6 = 0;
  return Float64Array.from(values, valueof === void 0 ? (v2) => sum7 += +v2 || 0 : (v2) => sum7 += +valueof(v2, index6++, values) || 0);
}
...
function fcumsum(values, valueof) {
  ...
  return Float64Array.from(
    values,
    valueof === void 0 ? (v2) => adder.add(+v2 || 0) : (v2) => adder.add(+valueof(v2, ++index6, values) || 0)
  );
}
...
function flatten(groups5, keys) {
  for (let i = 1, n = keys.length; i < n; ++i) {
    groups5 = groups5.flatMap((g) => g.pop().map(([key, value]) => [...g, key, value]));
  }
  return groups5;
}
...
function flatRollup(values, reduce5, ...keys) {
  return flatten(rollups(values, reduce5, ...keys), keys);
}
function rollup(values, reduce5, ...keys) {
  return nest(values, identity, reduce5, keys);
}
function rollups(values, reduce5, ...keys) {
  return nest(values, Array.from, reduce5, keys);
}
...
function nest(values, map10, reduce5, keys) {
  return function regroup(values2, i) {
    if (i >= keys.length)
      return reduce5(values2);
    const groups5 = new InternMap();
    const keyof2 = keys[i++];
    let index6 = -1;
    for (const value of values2) {
      const key = keyof2(value, ++index6, values2);
      const group5 = groups5.get(key);
      if (group5)
        group5.push(value);
      else
        groups5.set(key, [value]);
    }
    for (const [key, values3] of groups5) {
      groups5.set(key, regroup(values3, i));
    }
    return map10(groups5);
  }(values, 0);
}
...
function sort(values, ...F) {
  ...
  if (f && f.length !== 2 || F.length > 1) {
    const index6 = Uint32Array.from(values, (d, i) => i);
    if (F.length > 1) {
      ...
      f = values.map(f);
      index6.sort((i, j) => ascendingDefined(f[i], f[j]));
    }
    return permute(values, index6);
  }
...
function groupSort(values, reduce5, key) {
  return (reduce5.length !== 2 ? sort(rollup(values, reduce5, key), ([ak, av], [bk, bv]) => ascending(av, bv) || ascending(ak, bk)) : sort(group(values, key), ([ak, av], [bk, bv]) => reduce5(av, bv) || ascending(ak, bk))).map(([key2]) => key2);
}
...
function ticks(start2, stop, count6) {
  ...
  if (start2 === stop)
    return [start2];
  const reverse5 = stop < start2, [i1, i2, inc2] = reverse5 ? tickSpec(stop, start2, count6) : tickSpec(start2, stop, count6);
  if (!(i2 >= i1))
    return [];
  const n = i2 - i1 + 1, ticks5 = new Array(n);
  if (reverse5) {
    if (inc2 < 0)
      for (let i = 0; i < n; ++i)
        ticks5[i] = (i2 - i) / -inc2;
...
function tickStep(start2, stop, count6) {
  stop = +stop, start2 = +start2, count6 = +count6;
  const reverse5 = stop < start2, inc2 = reverse5 ? tickIncrement(stop, start2, count6) : tickIncrement(start2, stop, count6);
  return (reverse5 ? -1 : 1) * (inc2 < 0 ? 1 / -inc2 : inc2);
}

// node_modules/d3/node_modules/d3-array/src/nice.js
function nice(start2, stop, count6) {
  let prestep;
  while (true) {
    const step = tickIncrement(start2, stop, count6);
...
function bin() {
  ...
  function histogram(data) {
    ...
    var bins = new Array(m3 + 1), bin5;
    for (i = 0; i <= m3; ++i) {
      bin5 = bins[i] = [];
      bin5.x0 = i > 0 ? tz[i - 1] : x06;
      bin5.x1 = i < m3 ? tz[i] : x12;
    }
    if (isFinite(step)) {
      if (step > 0) {
        for (i = 0; i < n; ++i) {
...
function max(values, valueof) {
  ...
  if (valueof === void 0) {
    ...
    let index6 = -1;
    for (let value of values) {
      if ((value = valueof(value, ++index6, values)) != null && (max8 < value || max8 === void 0 && value >= value)) {
        max8 = value;
      }
    }
  }
...
function min(values, valueof) {
  ...
  if (valueof === void 0) {
    ...
    let index6 = -1;
    for (let value of values) {
      if ((value = valueof(value, ++index6, values)) != null && (min7 > value || min7 === void 0 && value >= value)) {
        min7 = value;
      }
    }
  }
...
function quickselect(array7, k2, left2 = 0, right2 = Infinity, compare) {
  k2 = Math.floor(k2);
  left2 = Math.floor(Math.max(0, left2));
  right2 = Math.floor(Math.min(array7.length - 1, right2));
  ...
  while (right2 > left2) {
    ...
    let i = left2;
    let j = right2;
    swap(array7, left2, k2);
    if (compare(array7[right2], t) > 0)
      swap(array7, left2, right2);
    while (i < j) {
      swap(array7, i, j), ++i, --j;
    ...
    if (compare(array7[left2], t) === 0)
      swap(array7, left2, j);
    else
      ++j, swap(array7, j, right2);
    if (j <= k2)
      left2 = j + 1;
    if (k2 <= j)
      right2 = j - 1;
...
function greatest(values, compare = ascending) {
  ...
  if (compare.length === 1) {
    ...
    for (const value of values) {
      if (defined ? compare(value, max8) > 0 : compare(value, value) === 0) {
        max8 = value;
        defined = true;
      }
    }
...
function quantileIndex(values, p, valueof = number) {
  if (isNaN(p = +p))
    return;
  numbers5 = Float64Array.from(values, (_, i2) => number(valueof(values[i2], i2, values)));
  if (p <= 0)
    return minIndex(numbers5);
  if (p >= 1)
    return maxIndex(numbers5);
  var numbers5, index6 = Uint32Array.from(values, (_, i2) => i2), j = numbers5.length - 1, i = Math.floor(j * p);
  quickselect(index6, i, 0, j, (i2, j2) => ascendingDefined(numbers5[i2], numbers5[j2]));
  i = greatest(index6.subarray(0, i + 1), (i2) => numbers5[i2]);
  return i >= 0 ? i : -1;
}

// node_modules/d3/node_modules/d3-array/src/threshold/freedmanDiaconis.js
function thresholdFreedmanDiaconis(values, min7, max8) {
  const c6 = count(values), d = quantile(values, 0.75) - quantile(values, 0.25);
  return c6 && d ? Math.ceil((max8 - min7) / (2 * d * Math.pow(c6, -1 / 3))) : 1;
}

// node_modules/d3/node_modules/d3-array/src/threshold/scott.js
function thresholdScott(values, min7, max8) {
  const c6 = count(values), d = deviation(values);
  return c6 && d ? Math.ceil((max8 - min7) * Math.cbrt(c6) / (3.49 * d)) : 1;
}
...
function mode(values, valueof) {
  ...
  if (valueof === void 0) {
    ...
    let index6 = -1;
    for (let value of values) {
      if ((value = valueof(value, ++index6, values)) != null && value >= value) {
        counts.set(value, (counts.get(value) || 0) + 1);
      }
    }
  }
  let modeValue;
  let modeCount = 0;
  for (const [value, count6] of counts) {
    if (count6 > modeCount) {
      modeCount = count6;
      modeValue = value;
    }
...
function pairs(values, pairof = pair) {
  ...
  for (const value of values) {
    if (first)
      pairs5.push(pairof(previous, value));
    previous = value;
    first = true;
  }
  return pairs5;
...
function least(values, compare = ascending) {
  ...
  if (compare.length === 1) {
    ...
    for (const value of values) {
      if (defined ? compare(value, min7) < 0 : compare(value, value) === 0) {
        min7 = value;
        defined = true;
      }
    }
...
function greatestIndex(values, compare = ascending) {
  ...
  for (const value of values) {
    ++index6;
    if (max8 < 0 ? compare(value, value) === 0 : compare(value, maxValue) > 0) {
      maxValue = value;
      max8 = index6;
    }
  }
...
function scan(values, compare) {
  const index6 = leastIndex(values, compare);
  return index6 < 0 ? void 0 : index6;
}
...
function transpose(matrix) {
  if (!(n = matrix.length))
    return [];
  for (var i = -1, m3 = min(matrix, length2), transpose5 = new Array(m3); ++i < m3; ) {
...
function map2(values, mapper) {
  ...
  return Array.from(values, (value, index6) => mapper(value, index6, values));
}
...
function reduce(values, reducer2, value) {
  ...
  while ({ done, value: next } = iterator.next(), !done) {
    value = reducer2(value, next, ++index6, values);
  }
  return value;
}
...
function axis(orient, scale2) {
  ...
  function axis2(context) {
    var values = tickValues == null ? scale2.ticks ? scale2.ticks.apply(scale2, tickArguments) : scale2.domain() : tickValues, format2 = tickFormat2 == null ? scale2.tickFormat ? scale2.tickFormat.apply(scale2, tickArguments) : identity_default : tickFormat2, spacing = Math.max(tickSizeInner, 0) + tickPadding, range7 = scale2.range(), range0 = +range7[0] + offset, range1 = +range7[range7.length - 1] + offset, position = (scale2.bandwidth ? center : number2)(scale2.copy(), offset), selection2 = context.selection ? context.selection() : context, path2 = selection2.selectAll(".domain").data([null]), tick = selection2.selectAll(".tick").data(values, scale2).order(), tickExit = tick.exit(), tickEnter = tick.enter().append("g").attr("class", "tick"), line = tick.select("line"), text = tick.select("text");
...
function select_default(select) {
  ...
  for (var groups5 = this._groups, m3 = groups5.length, subgroups = new Array(m3), j = 0; j < m3; ++j) {
    for (var group5 = groups5[j], n = group5.length, subgroup = subgroups[j] = new Array(n), node, subnode, i = 0; i < n; ++i) {
      if ((node = group5[i]) && (subnode = select.call(node, node.__data__, i, group5))) {
        if ("__data__" in node)
...
function selectAll_default(select) {
  ...
  for (var groups5 = this._groups, m3 = groups5.length, subgroups = [], parents = [], j = 0; j < m3; ++j) {
    for (var group5 = groups5[j], n = group5.length, node, i = 0; i < n; ++i) {
      if (node = group5[i]) {
        subgroups.push(select.call(node, node.__data__, i, group5));
        parents.push(node);
      }
    }
  }
...
function filter_default(match) {
  ...
  for (var groups5 = this._groups, m3 = groups5.length, subgroups = new Array(m3), j = 0; j < m3; ++j) {
    for (var group5 = groups5[j], n = group5.length, subgroup = subgroups[j] = [], node, i = 0; i < n; ++i) {
...
function data_default(value, key) {
  ...
  var bind = key ? bindKey : bindIndex, parents = this._parents, groups5 = this._groups;
  if (typeof value !== "function")
    value = constant_default(value);
  for (var m3 = groups5.length, update = new Array(m3), enter = new Array(m3), exit = new Array(m3), j = 0; j < m3; ++j) {
    var parent = parents[j], group5 = groups5[j], groupLength = group5.length, data = arraylike(value.call(parent, parent && parent.__data__, j, parents)), dataLength = data.length, enterGroup = enter[j] = new Array(dataLength), updateGroup = update[j] = new Array(dataLength), exitGroup = exit[j] = new Array(groupLength);
...
function order_default() {
  for (var groups5 = this._groups, j = -1, m3 = groups5.length; ++j < m3; ) {
    for (var group5 = groups5[j], i = group5.length - 1, next = group5[i], node; --i >= 0; ) {
      if (node = group5[i]) {
...
function sort_default(compare) {
  ...
  for (var groups5 = this._groups, m3 = groups5.length, sortgroups = new Array(m3), j = 0; j < m3; ++j) {
    for (var group5 = groups5[j], n = group5.length, sortgroup = sortgroups[j] = new Array(n), node, i = 0; i < n; ++i) {
      if (node = group5[i]) {
        sortgroup[i] = node;
      }
    }
...
function node_default() {
  for (var groups5 = this._groups, j = 0, m3 = groups5.length; j < m3; ++j) {
...
function each_default(callback) {
  for (var groups5 = this._groups, j = 0, m3 = groups5.length; j < m3; ++j) {
    for (var group5 = groups5[j], i = 0, n = group5.length, node; i < n; ++i) {
      if (node = group5[i])
...
var root = [null];
function Selection(groups5, parents) {
...
function hslConvert(o) {
  ...
  if (o instanceof Hsl)
    return o;
  o = o.rgb();
  var r = o.r / 255, g = o.g / 255, b = o.b / 255, min7 = Math.min(r, g, b), max8 = Math.max(r, g, b), h = NaN, s2 = max8 - min7, l = (max8 + min7) / 2;
  if (s2) {
    if (r === max8)
      h = (g - b) / s2 + (g < b) * 6;
    else if (g === max8)
      ...
      h = (r - g) / s2 + 4;
    s2 /= l < 0.5 ? max8 + min7 : 2 - max8 - min7;
    h *= 60;
  } else {
    s2 = l > 0 && l < 1 ? 0 : h;
  }
...
var ENDING = 5;
var ENDED = 6;
function schedule_default(node, name, id2, index6, group5, timing) {
...
function selectAll_default3(select) {
  ...
  for (var groups5 = this._groups, m3 = groups5.length, subgroups = [], parents = [], j = 0; j < m3; ++j) {
    for (var group5 = groups5[j], n = group5.length, node, i = 0; i < n; ++i) {
      if (node = group5[i]) {
        for (var children2 = select.call(node, node.__data__, i, group5), child, inherit2 = get2(node, id2), k2 = 0, l = children2.length; k2 < l; ++k2) {
...
function transition_default() {
  ...
  for (var groups5 = this._groups, m3 = groups5.length, j = 0; j < m3; ++j) {
    for (var group5 = groups5[j], n = group5.length, node, i = 0; i < n; ++i) {
      if (node = group5[i]) {
  ...
  return new Transition(groups5, this._parents, name, id1);
}
...
var id = 0;
function Transition(groups5, parents, name, id2) {
  this._groups = groups5;
  this._parents = parents;
  this._name = name;
  this._id = id2;
...
function transition_default2(name) {
  ...
  for (var groups5 = this._groups, m3 = groups5.length, j = 0; j < m3; ++j) {
    for (var group5 = groups5[j], n = group5.length, node, i = 0; i < n; ++i) {
      if (node = group5[i]) {
        schedule_default(node, name, id2, i, group5, timing || inherit(node, id2));
      }
    }
  }
  return new Transition(groups5, this._parents, name, id2);
}
...
function BrushEvent(type2, {
  ...
  Object.defineProperties(this, {
    ...
    mode: { value: mode5, enumerable: true, configurable: true },
    _: { value: dispatch2 }
  });
}
...
function brush(dim) {
  var extent5 = defaultExtent, filter6 = defaultFilter2, touchable = defaultTouchable2, keys = true, listeners = dispatch_default("start", "brush", "end"), handleSize = 6, touchending;
  ...
  brush2.move = function(group5, selection2, event) {
    if (group5.tween) {
  ...
  brush2.clear = function(group5, event) {
    brush2.move(group5, null, event);
  };
  function redraw() {
    var group5 = select_default2(this), selection2 = local2(this).selection;
    if (selection2) {
      group5.selectAll(".selection").style("display", null).attr("x", selection2[0][0]).attr("y", selection2[0][1]).attr("width", selection2[1][0] - selection2[0][0]).attr("height", selection2[1][1] - selection2[0][1]);
  ...
  Emitter.prototype = {
    ...
    start: function(event, mode5) {
      if (this.starting)
        this.starting = false, this.emit("start", event, mode5);
      else
        this.emit("brush", event);
      return this;
    },
    brush: function(event, mode5) {
      this.emit("brush", event, mode5);
      return this;
    },
    end: function(event, mode5) {
      if (--this.active === 0)
        delete this.state.emitter, this.emit("end", event, mode5);
      return this;
    },
    emit: function(type2, event, mode5) {
  ...
  function started(event) {
    ...
    if (!filter6.apply(this, arguments))
      return;
    var that = this, type2 = event.target.__data__.type, mode5 = (keys && event.metaKey ? type2 = "overlay" : type2) === "selection" ? MODE_DRAG : keys && event.altKey ? MODE_CENTER : MODE_HANDLE, signX = dim === Y ? null : signsX[type2], signY = dim === X ? null : signsY[type2], state = local2(that), extent6 = state.extent, selection2 = state.selection, W = extent6[0][0], w0, w1, N = extent6[0][1], n0, n1, E2 = extent6[1][0], e0, e1, S = extent6[1][1], s0, s1, dx = 0, dy = 0, moving, shifting = signX && signY && keys && event.shiftKey, lockX, lockY, points = Array.from(event.touches || [event], (t) => {
    ...
    var overlay = group5.selectAll(".overlay").attr("cursor", cursors[type2]);
    if (event.touches) {
      emit.moved = moved;
      emit.ended = ended;
    } else {
  ...
  brush2.extent = function(_) {
    return arguments.length ? (extent5 = typeof _ === "function" ? _ : constant_default4(number22(_)), brush2) : extent5;
  };
...
function chord(directed, transpose5) {
  var padAngle = 0, sortGroups = null, sortSubgroups = null, sortChords = null;
  function chord2(matrix) {
    var n = matrix.length, groupSums = new Array(n), groupIndex = range2(0, n), chords = new Array(n * n), groups5 = new Array(n), k2 = 0, dx;
    matrix = Float64Array.from({ length: n * n }, transpose5 ? (_, i) => matrix[i % n][i / n | 0] : (_, i) => matrix[i / n | 0][i % n]);
    for (let i = 0; i < n; ++i) {
      let x4 = 0;
      for (let j = 0; j < n; ++j)
    ...
    dx = k2 ? padAngle : tau2 / n;
    {
      ...
      for (const i of groupIndex) {
        ...
        if (directed) {
          ...
          groups5[i] = { index: i, startAngle: x06, endAngle: x4, value: groupSums[i] };
        } else {
...
function blurh2(blur6, T, S, w, h) {
  for (let y4 = 0, n = w * h; y4 < n; ) {
    blur6(T, S, y4, y4 += w, 1);
  }
}
function blurv2(blur6, T, S, w, h) {
  for (let x4 = 0, n = w * h; x4 < w; ++x4) {
    blur6(T, S, x4, x4 + n, w);
  }
}
...
var e22 = Math.sqrt(2);
function tickSpec2(start2, stop, count6) {
  const step = (stop - start2) / Math.max(0, count6), power = Math.floor(Math.log10(step)), error = step / Math.pow(10, power), factor = error >= e102 ? 10 : error >= e52 ? 5 : error >= e22 ? 2 : 1;
  let i1, i2, inc2;
  if (power < 0) {
    inc2 = Math.pow(10, -power) / factor;
    i1 = Math.round(start2 * inc2);
  ...
  if (i2 < i1 && 0.5 <= count6 && count6 < 2)
    return tickSpec2(start2, stop, count6 * 2);
  return [i1, i2, inc2];
}
function ticks2(start2, stop, count6) {
  ...
  if (start2 === stop)
    return [start2];
  const reverse5 = stop < start2, [i1, i2, inc2] = reverse5 ? tickSpec2(stop, start2, count6) : tickSpec2(start2, stop, count6);
  if (!(i2 >= i1))
    return [];
  const n = i2 - i1 + 1, ticks5 = new Array(n);
  if (reverse5) {
...
function tickIncrement2(start2, stop, count6) {
  stop = +stop, start2 = +start2, count6 = +count6;
  return tickSpec2(start2, stop, count6)[2];
}

// node_modules/d3-contour/node_modules/d3-array/src/nice.js
function nice2(start2, stop, count6) {
  let prestep;
  while (true) {
    const step = tickIncrement2(start2, stop, count6);
    if (step === prestep || step === 0 || !isFinite(step)) {
      return [start2, stop];
    } else if (step > 0) {
      start2 = Math.floor(start2 / step) * step;
...
function contours_default() {
  ...
  function isorings(values, value, callback) {
    ...
    function stitch(line) {
      var start2 = [line[0][0] + x4, line[0][1] + y4], end = [line[1][0] + x4, line[1][1] + y4], startIndex = index6(start2), endIndex = index6(end), f, g;
...
// node_modules/delaunator/node_modules/robust-predicates/esm/util.js
...
// node_modules/delaunator/node_modules/robust-predicates/esm/orient2d.js
...
// node_modules/delaunator/node_modules/robust-predicates/esm/orient3d.js
...
// node_modules/delaunator/node_modules/robust-predicates/esm/incircle.js
...
// node_modules/delaunator/node_modules/robust-predicates/esm/insphere.js
...
function collinear2(d) {
  ...
  for (let i = 0; i < triangles.length; i += 3) {
    const a4 = 2 * triangles[i], b = 2 * triangles[i + 1], c6 = 2 * triangles[i + 2], cross5 = (coords[c6] - coords[a4]) * (coords[b + 1] - coords[a4 + 1]) - (coords[b] - coords[a4]) * (coords[c6 + 1] - coords[a4 + 1]);
    if (cross5 > 1e-10)
      return false;
  }
  return true;
...
function autoType(object2) {
  for (var key in object2) {
    var value = object2[key].trim(), number8, m3;
    if (!value)
      value = null;
    else if (value === "true")
      value = true;
...
function link_default(links) {
  var id2 = index3, strength = defaultStrength, strengths, distance = constant_default7(30), distances, nodes, count6, bias, random, iterations2 = 1;
  if (links == null)
    links = [];
  function defaultStrength(link3) {
    return 1 / Math.min(count6[link3.source.index], count6[link3.target.index]);
  }
  ...
  function initialize() {
    ...
    for (i = 0, count6 = new Array(n); i < m3; ++i) {
      ...
      count6[link3.source.index] = (count6[link3.source.index] || 0) + 1;
      count6[link3.target.index] = (count6[link3.target.index] || 0) + 1;
    }
    for (i = 0, bias = new Array(m3); i < m3; ++i) {
      link3 = links[i], bias[i] = count6[link3.source.index] / (count6[link3.source.index] + count6[link3.target.index]);
    }
...
function rangeContains(range7, x4) {
  return range7[0] <= range7[1] ? range7[0] <= x4 && x4 <= range7[1] : x4 < range7[0] || range7[1] < x4;
}
...
function polygonContains_default(polygon, point6) {
  ...
  for (var i = 0, n = polygon.length; i < n; ++i) {
    ...
    for (var j = 0; j < m3; ++j, lambda04 = lambda12, sinPhi03 = sinPhi1, cosPhi03 = cosPhi1, point0 = point1) {
      ...
      sum7.add(atan2(k2 * sign3 * sin2(absDelta), cosPhi03 * cosPhi1 + k2 * cos2(absDelta)));
      angle2 += antimeridian ? delta + sign3 * tau5 : delta;
      if (antimeridian ^ lambda04 >= lambda ^ lambda12 >= lambda) {
        var arc = cartesianCross(cartesian(point0), cartesian(point1));
        cartesianNormalizeInPlace(arc);
        var intersection5 = cartesianCross(normal, arc);
        cartesianNormalizeInPlace(intersection5);
        var phiArc = (antimeridian ^ delta >= 0 ? -1 : 1) * asin(intersection5[2]);
  ...
  return (angle2 < -epsilon7 || angle2 < epsilon7 && sum7 < -epsilon22) ^ winding & 1;
}
...
function fitExtent(projection2, extent5, object2) {
  return fit(projection2, function(b) {
    var w = extent5[1][0] - extent5[0][0], h = extent5[1][1] - extent5[0][1], k2 = Math.min(w / (b[1][0] - b[0][0]), h / (b[1][1] - b[0][1])), x4 = +extent5[0][0] + (w - k2 * (b[1][0] + b[0][0])) / 2, y4 = +extent5[0][1] + (h - k2 * (b[1][1] + b[0][1])) / 2;
...
function projectionMutator(projectAt) {
  ...
  projection2.fitExtent = function(extent5, object2) {
    return fitExtent(projection2, extent5, object2);
  };
...
function count3(node) {
  var sum7 = 0, children2 = node.children, i = children2 && children2.length;
  if (!i)
    sum7 = 1;
  else
    while (--i >= 0)
      sum7 += children2[i].value;
  node.value = sum7;
}
...
function each_default2(callback, that) {
  let index6 = -1;
  for (const node of this) {
    callback.call(that, node, ++index6, this);
  }
  return this;
}
...
function eachBefore_default(callback, that) {
  var node = this, nodes = [node], children2, i, index6 = -1;
  while (node = nodes.pop()) {
    callback.call(that, node, ++index6, this);
...
function eachAfter_default(callback, that) {
  var node = this, nodes = [node], next = [], children2, i, n, index6 = -1;
  while (node = nodes.pop()) {
    next.push(node);
    if (children2 = node.children) {
...
function find_default2(callback, that) {
  let index6 = -1;
  for (const node of this) {
    if (callback.call(that, node, ++index6, this)) {
      return node;
    }
  }
}
...
function sum_default(value) {
  return this.eachAfter(function(node) {
    var sum7 = +value(node.data) || 0, children2 = node.children, i = children2 && children2.length;
    while (--i >= 0)
      sum7 += children2[i].value;
    node.value = sum7;
  });
...
function binary_default(parent, x06, y06, x12, y12) {
  var nodes = parent.children, i, n = nodes.length, sum7, sums = new Array(n + 1);
...
function computeUpperHullIndexes(points) {
  const n = points.length, indexes5 = [0, 1];
  let size = 2, i;
  for (i = 2; i < n; ++i) {
    while (size > 1 && cross_default(points[indexes5[size - 2]], points[indexes5[size - 1]], points[i]) <= 0)
      --size;
    indexes5[size++] = i;
  }
...
var uniform_default = function sourceRandomUniform(source) {
  function randomUniform(min7, max8) {
...
var int_default = function sourceRandomInt(source) {
  function randomInt(min7, max8) {
    if (arguments.length < 2)
      max8 = min7, min7 = 0;
    min7 = Math.floor(min7);
    max8 = Math.floor(max8) - min7;
    return function() {
      return Math.floor(source() * max8 + min7);
    };
  }
...
var irwinHall_default = function sourceRandomIrwinHall(source) {
  function randomIrwinHall(n) {
    if ((n = +n) <= 0)
      return () => 0;
    return function() {
      for (var sum7 = 0, i = n; i > 1; --i)
        sum7 += source();
      return sum7 + i * source();
    };
  }
...
function blurh3(blur6, T, S, w, h) {
  for (let y4 = 0, n = w * h; y4 < n; ) {
    blur6(T, S, y4, y4 += w, 1);
  }
}
function blurv3(blur6, T, S, w, h) {
  for (let x4 = 0, n = w * h; x4 < w; ++x4) {
    blur6(T, S, x4, x4 + n, w);
  }
}
...
var e23 = Math.sqrt(2);
function tickSpec3(start2, stop, count6) {
  const step = (stop - start2) / Math.max(0, count6), power = Math.floor(Math.log10(step)), error = step / Math.pow(10, power), factor = error >= e103 ? 10 : error >= e53 ? 5 : error >= e23 ? 2 : 1;
  let i1, i2, inc2;
  if (power < 0) {
    inc2 = Math.pow(10, -power) / factor;
    i1 = Math.round(start2 * inc2);
  ...
  if (i2 < i1 && 0.5 <= count6 && count6 < 2)
    return tickSpec3(start2, stop, count6 * 2);
  return [i1, i2, inc2];
}
function ticks3(start2, stop, count6) {
  stop = +stop, start2 = +start2, count6 = +count6;
  if (!(count6 > 0))
    return [];
  if (start2 === stop)
    return [start2];
  const reverse5 = stop < start2, [i1, i2, inc2] = reverse5 ? tickSpec3(stop, start2, count6) : tickSpec3(start2, stop, count6);
  if (!(i2 >= i1))
    return [];
  const n = i2 - i1 + 1, ticks5 = new Array(n);
  if (reverse5) {
...
function tickIncrement3(start2, stop, count6) {
  stop = +stop, start2 = +start2, count6 = +count6;
  return tickSpec3(start2, stop, count6)[2];
}
function tickStep3(start2, stop, count6) {
  stop = +stop, start2 = +start2, count6 = +count6;
  const reverse5 = stop < start2, inc2 = reverse5 ? tickIncrement3(stop, start2, count6) : tickIncrement3(start2, stop, count6);
  return (reverse5 ? -1 : 1) * (inc2 < 0 ? 1 / -inc2 : inc2);
}
...
function quickselect3(array7, k2, left2 = 0, right2 = Infinity, compare) {
  k2 = Math.floor(k2);
  left2 = Math.floor(Math.max(0, left2));
  right2 = Math.floor(Math.min(array7.length - 1, right2));
  if (!(left2 <= k2 && k2 <= right2))
  ...
  while (right2 > left2) {
    if (right2 - left2 > 600) {
      ...
      const newLeft = Math.max(left2, Math.floor(k2 - m3 * s2 / n + sd));
      const newRight = Math.min(right2, Math.floor(k2 + (n - m3) * s2 / n + sd));
      quickselect3(array7, k2, newLeft, newRight, compare);
    }
    const t = array7[k2];
    let i = left2;
    let j = right2;
    swap3(array7, left2, k2);
    if (compare(array7[right2], t) > 0)
      swap3(array7, left2, right2);
    while (i < j) {
    ...
    if (compare(array7[left2], t) === 0)
      swap3(array7, left2, j);
    else
      ++j, swap3(array7, j, right2);
    if (j <= k2)
      left2 = j + 1;
    if (k2 <= j)
      right2 = j - 1;
...
function ordinal() {
  var index6 = new InternMap(), domain = [], range7 = [], unknown = implicit;
  function scale2(d) {
    let i = index6.get(d);
    if (i === void 0) {
      if (unknown !== implicit)
  ...
  scale2.range = function(_) {
    return arguments.length ? (range7 = Array.from(_), scale2) : range7.slice();
  };
...
function band() {
  ...
  function rescale() {
    var n = domain().length, reverse5 = r1 < r0, start2 = reverse5 ? r1 : r0, stop = reverse5 ? r0 : r1;
...
function bimap(domain, range7, interpolate) {
...
function polymap(domain, range7, interpolate) {
  var j = Math.min(domain.length, range7.length) - 1, d = new Array(j), r = new Array(j), i = -1;
  ...
  while (++i < j) {
    ...
    r[i] = interpolate(range7[i], range7[i + 1]);
  }
  return function(x4) {
...
function transformer2() {
  var domain = unit, range7 = unit, interpolate = value_default, transform2, untransform, unknown, clamp = identity5, piecewise2, output, input;
  function rescale() {
    var n = Math.min(domain.length, range7.length);
    if (clamp !== identity5)
  ...
  function scale2(x4) {
    return x4 == null || isNaN(x4 = +x4) ? unknown : (output || (output = piecewise2(domain.map(transform2), range7, interpolate)))(transform2(clamp(x4)));
  }
  scale2.invert = function(y4) {
    return clamp(untransform((input || (input = piecewise2(range7, domain.map(transform2), number_default)))(y4)));
  };
  scale2.domain = function(_) {
  ...
  scale2.rangeRound = function(_) {
    return range7 = Array.from(_), interpolate = round_default, rescale();
  };
...
function tickFormat(start2, stop, count6, specifier) {
  var step = tickStep3(start2, stop, count6), precision;
...
function linearish(scale2) {
  ...
  scale2.ticks = function(count6) {
    var d = domain();
    return ticks3(d[0], d[d.length - 1], count6 == null ? 10 : count6);
  };
  scale2.tickFormat = function(count6, specifier) {
    var d = domain();
    return tickFormat(d[0], d[d.length - 1], count6 == null ? 10 : count6, specifier);
  };
  scale2.nice = function(count6) {
    if (count6 == null)
      count6 = 10;
    var d = domain();
    var i0 = 0;
    ...
    while (maxIter-- > 0) {
      step = tickIncrement3(start2, stop, count6);
      if (step === prestep) {
        d[i0] = start2;
        d[i1] = stop;
        return domain(d);
...
function loggish(transform2) {
  ...
  scale2.tickFormat = (count6, specifier) => {
    if (count6 == null)
      count6 = 10;
    if (specifier == null)
      specifier = base === 10 ? "s" : ",";
    ...
    if (count6 === Infinity)
      return specifier;
    const k2 = Math.max(1, base * count6 / scale2.ticks().length);
    return (d) => {
...
function radial() {
  var squared = continuous(), range7 = [0, 1], round = false, unknown;
  function scale2(x4) {
  ...
  scale2.range = function(_) {
    return arguments.length ? (squared.range((range7 = Array.from(_, number5)).map(square)), scale2) : range7.slice();
  };
  scale2.rangeRound = function(_) {
  ...
  scale2.copy = function() {
    return radial(squared.domain(), range7).round(round).clamp(squared.clamp()).unknown(unknown);
  };
  initRange.apply(scale2, arguments);
  return linearish(scale2);
}
...
function quantile4() {
  ...
  function rescale() {
    ...
    while (++i < n)
      thresholds[i - 1] = quantileSorted3(domain, i / n);
    return scale2;
  }
  function scale2(x4) {
    return x4 == null || isNaN(x4 = +x4) ? unknown : range7[bisect_default3(thresholds, x4)];
  }
  scale2.invertExtent = function(y4) {
    var i = range7.indexOf(y4);
    return i < 0 ? [NaN, NaN] : [
  ...
  scale2.copy = function() {
    return quantile4().domain(domain).range(range7).unknown(unknown);
  };
  return initRange.apply(scale2, arguments);
}
...
function quantize() {
  var x06 = 0, x12 = 1, n = 1, domain = [0.5], range7 = [0, 1], unknown;
  function scale2(x4) {
    return x4 != null && x4 <= x4 ? range7[bisect_default3(domain, x4, 0, n)] : unknown;
  }
  function rescale() {
    var i = -1;
    domain = new Array(n);
...
function threshold() {
  var domain = [0.5], range7 = [0, 1], unknown, n = 1;
  function scale2(x4) {
    return x4 != null && x4 <= x4 ? range7[bisect_default3(domain, x4, 0, n)] : unknown;
  }
  scale2.domain = function(_) {
    return arguments.length ? (domain = Array.from(_), n = Math.min(domain.length, range7.length - 1), scale2) : domain.slice();
  };
  scale2.range = function(_) {
    return arguments.length ? (range7 = Array.from(_), n = Math.min(domain.length, range7.length - 1), scale2) : range7.slice();
  };
  scale2.invertExtent = function(y4) {
    var i = range7.indexOf(y4);
...
function timeInterval(floori, offseti, count6, field) {
  ...
  interval2.range = (start2, stop, step) => {
    ...
    let previous;
    do
      range7.push(previous = /* @__PURE__ */ new Date(+start2)), offseti(start2, step), floori(start2);
...
function descending4(a4, b) {
  return a4 == null || b == null ? NaN : b < a4 ? -1 : b > a4 ? 1 : b >= a4 ? 0 : NaN;
}

// node_modules/d3-time/node_modules/d3-array/src/bisector.js
function bisector4(f) {
  let compare1, compare2, delta;
  if (f.length !== 2) {
    compare1 = ascending5;
    compare2 = (d, x4) => ascending5(f(d), x4);
    delta = (d, x4) => f(d) - x4;
  } else {
    compare1 = f === ascending5 || f === descending4 ? f : zero5;
    compare2 = f;
    delta = f;
  }
  function left2(a4, x4, lo = 0, hi = a4.length) {
...
var ascendingBisect4 = bisector4(ascending5);
var bisectRight4 = ascendingBisect4.right;
var bisectLeft4 = ascendingBisect4.left;
var bisectCenter4 = bisector4(number6).center;
...
function Blur24(blur6) {
  return function(data, rx, ry = rx) {
    ...
    const temp = values.slice();
    if (blurx && blury) {
      blurh4(blurx, temp, values, width, height);
      blurh4(blurx, values, temp, width, height);
      blurh4(blurx, temp, values, width, height);
      blurv4(blury, values, temp, width, height);
      blurv4(blury, temp, values, width, height);
      blurv4(blury, values, temp, width, height);
    } else if (blurx) {
      blurh4(blurx, values, temp, width, height);
      blurh4(blurx, temp, values, width, height);
...
function blurh4(blur6, T, S, w, h) {
  for (let y4 = 0, n = w * h; y4 < n; ) {
    blur6(T, S, y4, y4 += w, 1);
  }
}
function blurv4(blur6, T, S, w, h) {
  for (let x4 = 0, n = w * h; x4 < w; ++x4) {
    blur6(T, S, x4, x4 + n, w);
  }
}
...
var e24 = Math.sqrt(2);
function tickSpec4(start2, stop, count6) {
  const step = (stop - start2) / Math.max(0, count6), power = Math.floor(Math.log10(step)), error = step / Math.pow(10, power), factor = error >= e104 ? 10 : error >= e54 ? 5 : error >= e24 ? 2 : 1;
  let i1, i2, inc2;
  if (power < 0) {
    inc2 = Math.pow(10, -power) / factor;
    i1 = Math.round(start2 * inc2);
  ...
  if (i2 < i1 && 0.5 <= count6 && count6 < 2)
    return tickSpec4(start2, stop, count6 * 2);
  return [i1, i2, inc2];
}
function tickIncrement4(start2, stop, count6) {
  stop = +stop, start2 = +start2, count6 = +count6;
  return tickSpec4(start2, stop, count6)[2];
}
function tickStep4(start2, stop, count6) {
  stop = +stop, start2 = +start2, count6 = +count6;
  const reverse5 = stop < start2, inc2 = reverse5 ? tickIncrement4(stop, start2, count6) : tickIncrement4(start2, stop, count6);
  return (reverse5 ? -1 : 1) * (inc2 < 0 ? 1 / -inc2 : inc2);
}
...
function ticker(year, month, week, day, hour, minute) {
  ...
  function tickInterval(start2, stop, count6) {
    ...
    if (i === tickIntervals.length)
      return year.every(tickStep4(start2 / durationYear, stop / durationYear, count6));
    if (i === 0)
      return millisecond.every(Math.max(tickStep4(start2, stop, count6), 1));
...
function calendar(ticks5, tickInterval, year, month, week, day, hour, minute, second2, format2) {
  ...
  scale2.domain = function(_) {
    return arguments.length ? domain(Array.from(_, number7)) : domain().map(date);
  };
  scale2.ticks = function(interval2) {
    var d = domain();
    return ticks5(d[0], d[d.length - 1], interval2 == null ? 10 : interval2);
  };
  scale2.tickFormat = function(count6, specifier) {
  ...
  scale2.copy = function() {
    return copy(scale2, calendar(ticks5, tickInterval, year, month, week, day, hour, minute, second2, format2));
  };
  return scale2;
}
function time() {
...
function transformer3() {
  var x06 = 0, x12 = 1, t03, t13, k10, transform2, interpolator = identity5, clamp = false, unknown;
  function scale2(x4) {
...
function sequentialQuantile() {
  ...
  function scale2(x4) {
    if (x4 != null && !isNaN(x4 = +x4))
      return interpolator((bisect_default3(domain, x4, 1) - 1) / (domain.length - 1));
  }
  scale2.domain = function(_) {
    if (!arguments.length)
      return domain.slice();
...
function transformer4() {
  var x06 = 0, x12 = 0.5, x22 = 1, s2 = 1, t03, t13, t22, k10, k21, interpolator = identity5, transform2, clamp = false, unknown;
  function scale2(x4) {
...
function cornerTangents(x06, y06, x12, y12, r1, rc, cw) {
  var x01 = x06 - x12, y01 = y06 - y12, lo = (cw ? rc : -rc) / sqrt3(x01 * x01 + y01 * y01), ox = lo * y01, oy = -lo * x01, x11 = x06 + ox, y11 = y06 + oy, x10 = x12 + ox, y10 = y12 + oy, x004 = (x11 + x10) / 2, y004 = (y11 + y10) / 2, dx = x10 - x11, dy = y10 - y11, d2 = dx * dx + dy * dy, r = r1 - rc, D3 = x11 * y10 - x10 * y11, d = (dy < 0 ? -1 : 1) * sqrt3(max7(0, r * r * d2 - D3 * D3)), cx0 = (D3 * dy - dx * d) / d2, cy0 = (-D3 * dx - dy * d) / d2, cx1 = (D3 * dy + dx * d) / d2, cy1 = (-D3 * dx + dy * d) / d2, dx0 = cx0 - x004, dy0 = cy0 - y004, dx1 = cx1 - x004, dy1 = cy1 - y004;
...
function arc_default() {
  ...
  function arc() {
    ...
    if (!(r1 > epsilon8))
      ...
      var a01 = a0, a11 = a1, a00 = a0, a10 = a1, da0 = da2, da1 = da2, ap = padAngle.apply(this, arguments) / 2, rp = ap > epsilon8 && (padRadius ? +padRadius.apply(this, arguments) : sqrt3(r0 * r0 + r1 * r1)), rc = min6(abs4(r1 - r0) / 2, +cornerRadius.apply(this, arguments)), rc0 = rc, rc1 = rc, t03, t13;
      if (rp > epsilon8) {
...
function pie_default() {
  ...
  function pie(data) {
    var i, n = (data = array_default3(data)).length, j, k2, sum7 = 0, index6 = new Array(n), arcs = new Array(n), a0 = +startAngle.apply(this, arguments), da2 = Math.min(tau6, Math.max(-tau6, endAngle.apply(this, arguments) - a0)), a1, p = Math.min(Math.abs(da2) / n, padAngle.apply(this, arguments)), pa = p * (da2 < 0 ? -1 : 1), v2;
    for (i = 0; i < n; ++i) {
    ...
    for (i = 0, k2 = sum7 ? (da2 - n * pa) / sum7 : 0; i < n; ++i, a0 = a1) {
      j = index6[i], v2 = arcs[j], a1 = a0 + (v2 > 0 ? v2 * k2 : 0) + pa, arcs[j] = {
        data: data[j],
        index: i,
        value: v2,
        startAngle: a0,
...
function link2(curve) {
  ...
  function link3() {
    let buffer;
    const argv = slice7.call(arguments);
...
var asterisk_default = {
  draw(context, size) {
    const r = sqrt3(size + min6(size / 28, 0.75)) * 0.59436;
    const t = r / 2;
    const u4 = t * sqrt32;
    context.moveTo(0, r);
    context.lineTo(0, -r);
...
var plus_default = {
  draw(context, size) {
    const r = sqrt3(size - min6(size / 7, 2)) * 0.87559;
    context.moveTo(-r, 0);
    context.lineTo(r, 0);
    context.moveTo(0, r);
    context.lineTo(0, -r);
...
var times_default = {
  draw(context, size) {
    const r = sqrt3(size - min6(size / 6, 1.7)) * 0.6189;
    context.moveTo(-r, -r);
    context.lineTo(r, r);
    context.moveTo(-r, r);
    context.lineTo(r, -r);
...
function insideOut_default(series) {
  var n = series.length, i, j, sums = series.map(sum6), order = appearance_default(series), top2 = 0, bottom2 = 0, tops = [], bottoms = [];
  for (i = 0; i < n; ++i) {
    j = order[i];
    if (top2 < bottom2) {
      top2 += sums[j];
...
function zoom_default2() {
  ...
  zoom.scaleTo = function(selection2, k2, p, event) {
    zoom.transform(selection2, function() {
      var e = extent5.apply(this, arguments), t03 = this.__zoom, p02 = p == null ? centroid(e) : typeof p === "function" ? p.apply(this, arguments) : p, p1 = t03.invert(p02), k1 = typeof k2 === "function" ? k2.apply(this, arguments) : k2;
  ...
  zoom.translateTo = function(selection2, x4, y4, p, event) {
    zoom.transform(selection2, function() {
      ...
      return constrain(identity8.translate(p02[0], p02[1]).scale(t.k).translate(
        typeof x4 === "function" ? -x4.apply(this, arguments) : -x4,
        typeof y4 === "function" ? -y4.apply(this, arguments) : -y4
      ), e, translateExtent);
    }, p, event);
  ...
  function schedule(transition2, transform2, point6, event) {
    transition2.on("start.zoom", function() {
      ...
      var that = this, args = arguments, g = gesture(that, args).event(event), e = extent5.apply(that, args), p = point6 == null ? centroid(e) : typeof point6 === "function" ? point6.apply(that, args) : point6, w = Math.max(e[1][0] - e[0][0], e[1][1] - e[0][1]), a4 = that.__zoom, b = typeof transform2 === "function" ? transform2.apply(that, args) : transform2, i = interpolate(a4.invert(p).concat(w / a4.k), b.invert(p).concat(w / b.k));
      return function(t) {
        if (t === 1)
          t = b;
        else {
  ...
  function dblclicked(event, ...args) {
    if (!filter6.apply(this, arguments))
      return;
    var t03 = this.__zoom, p02 = pointer_default(event.changedTouches ? event.changedTouches[0] : event, this), p1 = t03.invert(p02), k1 = t03.k * (event.shiftKey ? 0.5 : 2), t13 = constrain(translate(scale2(t03, k1), p02, p1), extent5.apply(this, args), translateExtent);
    noevent_default3(event);
    if (duration > 0)
  ...
  zoom.filter = function(_) {
    return arguments.length ? (filter6 = typeof _ === "function" ? _ : constant_default11(!!_), zoom) : filter6;
  };
